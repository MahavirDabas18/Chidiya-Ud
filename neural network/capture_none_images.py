...
cv2.namedWindow("none images")

img_counter = 0
num_samples=1000 #the max number of samples to be captured
path = 'images'
img_class_path = os.path.join(path, "none")
...
while True:
    ...
    frame = cv2.putText(frame, 'None Images: {}/{}  Space: Capture, Esc: Escape'.format(img_counter,num_samples), org, font, 
                   fontScale, color, thickness, cv2.LINE_AA)
    
    cv2.imshow("none images", frame) #shows the user the whole frame being captured by the cv2 window
    ...
    if k%256 == 27: #escape key for quitting 
        ...
        img_name = "none_{}.png".format(img_counter) #defining the image name
